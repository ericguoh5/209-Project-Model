    ...
    def __init__(self, in_channels, out_channels, skip_channels=0, use_batchnorm=True):
        ...
        if skip_channels > 0:
            self.att = Attention_block(
                F_g=in_channels,
                F_l=skip_channels,
                F_int=max(1, int(skip_channels / 2))
            )
...
class DecoderCup(nn.Module):
    ...
    def forward(self, hidden_states, features=None, return_features=False):
        ...
        ds_features = []  # for deep supervision
...
class VisionTransformer(nn.Module):
    def __init__(self, config, img_size=224, num_classes=21843, zero_head=False, vis=False):
        ...
        if self.deep_supervision:
            dec_ch = list(config.decoder_channels)
            n_classes = config['n_classes']

            self.aux_seg_heads = nn.ModuleList([
                SegmentationHead(dec_ch[0], n_classes, kernel_size=3, upsampling=8),
                SegmentationHead(dec_ch[1], n_classes, kernel_size=3, upsampling=4),
                SegmentationHead(dec_ch[2], n_classes, kernel_size=3, upsampling=2),
            ])
        else:
            self.aux_seg_heads = None
    ...
    def forward(self, x):
        ...
        if self.deep_supervision:
            dec_out, ds_features = self.decoder(x, features, return_features=True)
        else:
            dec_out = self.decoder(x, features, return_features=False)
            ds_features = None

        logits = self.segmentation_head(dec_out)
        edge_logits = self.edge_head(dec_out)

        if self.deep_supervision and ds_features is not None and self.aux_seg_heads is not None:
            aux_logits = []
            for i in range(min(3, len(ds_features))):
                aux_logits.append(self.aux_seg_heads[i](ds_features[i]))

            return logits, edge_logits, aux_logits
